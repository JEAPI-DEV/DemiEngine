...
| Networking | Experimental | Versioned network contracts, bounded declared messages and schemas, server-issued IDs, ownership generations and disconnect policy, reflected replicated fields, late-join state, reconnect/fault-test primitives, dedicated-server startup, optional lobby package, ENet, and TLS/DTLS transport; prediction and reconciliation are deferred |
...
| Asset streaming and generated atlases | Stable | Scene-rooted addressable groups, async progress/cancellation/reference counting, native GPU/audio ownership, startup and atomic transition activation, watched-reload rollback, low-memory/recovery hooks, Lua `Assets`, deterministic incremental cook keys/cache reports, texture/font atlas generation, and locked package provenance |
