...
function Player3D:on_create()
  ...
  self.camera_id = Entity.find("Main Camera 3D")
  self.camera_yaw = 0.0
...
function Player3D:on_update(dt)
  ...
  if Input.is_down("q") then
    self.camera_yaw = self.camera_yaw + self.rotation_speed * dt
  end
  if Input.is_down("e") then
    self.camera_yaw = self.camera_yaw - self.rotation_speed * dt
  end

  if self.camera_id ~= nil then
    local px, py, pz = Transform3D.get_position(self.entity_id)
    local radius = 12.0
    local cam_x = px + math.sin(self.camera_yaw) * radius
    local cam_z = pz + math.cos(self.camera_yaw) * radius
    Transform3D.set_position(self.camera_id, cam_x, py + 6.0, cam_z)
    Transform3D.set_rotation(self.camera_id, 0.5, self.camera_yaw, 0.0)
  end
