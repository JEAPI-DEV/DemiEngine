...
function PlayerCamera:update_walk(world, dt)
  ...
  local speed = Input.down("sprint") and self.sprint_speed or self.walk_speed
  local move_x, move_z = move_vector(self.yaw, right, forward)
  self:move_horizontal(world, move_x * speed * dt, move_z * speed * dt)

  if Input.pressed("jump") and self.grounded then
...
function PlayerCamera:update_fly(dt)
  local right = Input.value("move_right")
  local forward = Input.value("move_forward")
  local vertical = Input.value("move_vertical")
  local speed = Input.down("sprint") and self.fly_fast_speed or self.fly_speed
  local move_x, move_z = move_vector(self.yaw, right, forward)
...
function PlayerCamera:on_update(dt)
  ...
  if Input.pressed("toggle_fly") then
  ...
  local right = Input.value("move_right")
  local forward = Input.value("move_forward")
  local vertical = self.fly_mode and Input.value("move_vertical") or 0.0
