  ...
  for (const AssetManifest &asset : registry.assets) {
    ...
    if (asset.type == "AudioClip") {
      ...
      Diagnostics audioDiagnostics =
          validateAudioClipSettings(settings, asset.manifestPath);
      diagnostics.insert(diagnostics.end(), audioDiagnostics.begin(),
                         audioDiagnostics.end());
    }
...
Diagnostics validateModelAnimationSettings(const nlohmann::json &settings,
  ...
  for (const auto &clip : *clips) {
    const std::string name = clip.is_object() ? clip.value("name", "") : "";
    ...
    if (!clipSkeleton.empty() && skeleton.empty())
      ...
      diagnostics.push_back(
          {.severity = Severity::Error,
           ...
           .message = "Model animation clips reference different skeletons.",
           .path = path.string(),
           .suggestion = "Retarget clips to one skeleton before importing."});
  }
  return diagnostics;
}

Diagnostics validateAudioClipSettings(const nlohmann::json &settings,
                                      const std::filesystem::path &path) {
  Diagnostics diagnostics;
