...
std::vector<Vec3> luaVec3ArrayField(const sol::table table,
  ...
  for (const auto &entry : array) {
    ...
    values.push_back(Vec3{.x = value.get_or(1, 0.0F),
...
std::vector<Vec2> luaVec2ArrayField(const sol::table table,
                                    const char *fieldName) {
  ...
  for (const auto &entry : array) {
    ...
    const sol::table value = entry.second.as<sol::table>();
    values.push_back(
        Vec2{.x = value.get_or(1, 0.0F), .y = value.get_or(2, 0.0F)});
  }
  return values;
}
...
Entity luaParseEntitySpec(const std::string &entityId, const sol::table spec) {
  ...
  const sol::table components = componentsObject.is<sol::table>()
                                    ? componentsObject.as<sol::table>()
                                    : spec;

  if (const sol::table transform = componentTable(components, "Transform2D");
      transform.valid()) {
  ...
  if (const sol::table transform = componentTable(components, "Transform3D");
      transform.valid()) {
    entity.setComponent<Transform3DComponent>(Transform3DComponent{
        .parent = transform.get_or("parent", std::string{}),
  ...
  if (const sol::table rigidbody = componentTable(components, "Rigidbody2D");
      rigidbody.valid()) {
    entity.setComponent<Rigidbody2DComponent>(Rigidbody2DComponent{
  ...
  if (const sol::table collider = componentTable(components, "BoxCollider2D");
      collider.valid()) {
    entity.setComponent<BoxCollider2DComponent>(BoxCollider2DComponent{
  ...
  if (const sol::table mesh = componentTable(components, "MeshRenderer");
      mesh.valid()) {
    entity.setComponent<MeshRendererComponent>(MeshRendererComponent{
  ...
  if (const sol::table collider = componentTable(components, "BoxCollider3D");
      collider.valid()) {
    entity.setComponent<BoxCollider3DComponent>(BoxCollider3DComponent{
  ...
  if (const sol::table collider =
          componentTable(components, "SphereCollider3D");
      collider.valid()) {
    entity.setComponent<SphereCollider3DComponent>(SphereCollider3DComponent{
  ...
  if (const sol::table rigidbody = componentTable(components, "Rigidbody3D");
      rigidbody.valid()) {
    entity.setComponent<Rigidbody3DComponent>(Rigidbody3DComponent{
        .bodyType = rigidbody.get_or("body_type", std::string("dynamic")),
  ...
  if (const sol::table audioSource = componentTable(components, "AudioSource");
      audioSource.valid()) {
    entity.setComponent<AudioSourceComponent>(AudioSourceComponent{
  ...
  if (const sol::table audioListener =
          componentTable(components, "AudioListener");
      audioListener.valid()) {
    entity.setComponent<AudioListenerComponent>(AudioListenerComponent{
        .primary = audioListener.get_or("primary", true)});
  }

  if (const sol::table videoPlayer = componentTable(components, "VideoPlayer");
      videoPlayer.valid()) {
    entity.setComponent<VideoPlayerComponent>(VideoPlayerComponent{
