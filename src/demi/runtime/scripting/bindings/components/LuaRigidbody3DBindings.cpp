...
void LuaRigidbody3DBindingModule::install(LuaScriptHost &host,
  ...
  body.set_function("set_continuous",
                    [&host](const std::string &id, bool continuous) {
                      return host.setRigidbodyContinuous3D(id, continuous);
                    });
  body.set_function("set_report_contacts", [&host](const std::string &id,
                                                   bool reportContacts) {
    return host.setRigidbodyReportContacts3D(id, reportContacts);
  });
