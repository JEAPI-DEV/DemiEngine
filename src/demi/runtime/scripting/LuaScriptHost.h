...
public:
  ...
  [[nodiscard]] std::filesystem::path resolveProjectPath(const std::string& path) const;
  ...
  [[nodiscard]] bool networkHostSecure(std::uint16_t port, const std::string& certificate, const std::string& privateKey, std::uint32_t maxPeers);
  [[nodiscard]] bool networkConnect(const std::string& address, std::uint16_t port);
  [[nodiscard]] bool networkConnectSecure(const std::string& address, std::uint16_t port, const std::string& trustedCertificate, const std::string& serverName);
