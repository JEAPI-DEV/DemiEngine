...
void luaConfigurePackagePath(lua_State* state, const ProjectData& project);
	...
	[[nodiscard]] std::vector<LuaActionHandler> parseActionHandlers(const std::filesystem::path& path);
	[[nodiscard]] std::vector<LuaEventHandler> parseEventHandlers(const std::filesystem::path& path);
	[[nodiscard]] std::string moduleNameFromProjectEntry(std::string module);
	[[nodiscard]] std::string projectEntryToScriptUri(const std::string& module);
