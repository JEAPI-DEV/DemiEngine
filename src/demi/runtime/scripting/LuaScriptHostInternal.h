...
void luaCallUiEvent(lua_State* state, int tableRef, const char* functionName, const ui::UiNode& node, Vec2 mousePosition, const std::filesystem::path& path);
void luaCallActionEvent(lua_State* state, int tableRef, const std::string& functionName, const ui::UiNode& node, Vec2 mousePosition, const std::filesystem::path& path);
void luaCallModuleActionEvent(lua_State* state, const std::string& moduleName, const std::string& functionName, const ui::UiNode& node, Vec2 mousePosition, const std::filesystem::path& path);
