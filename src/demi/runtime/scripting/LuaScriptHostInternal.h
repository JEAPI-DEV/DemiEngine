...
luaResolveScriptPath(const ProjectData &project, const std::string &module);
...
void luaCallTypedUiEvent(lua_State *state, int tableRef,
                         ...
                         const std::filesystem::path &path);
void luaCallActionEvent(lua_State *state, int tableRef,
                        const std::string &functionName, const ui::UiNode &node,
                        Vec2 mousePosition, const std::filesystem::path &path);
void luaCallModuleActionEvent(lua_State *state, const std::string &moduleName,
                              const std::string &functionName,
                              const ui::UiNode &node, Vec2 mousePosition,
                              const std::filesystem::path &path);
void luaCallScriptEvent(lua_State *state, int tableRef,
                        const std::string &functionName, int payloadIndex,
                        const std::filesystem::path &path,
...
void luaCallModuleEvent(lua_State *state, const std::string &moduleName,
                        ...
                        const std::filesystem::path &path,
                        const std::string &eventName);
[[nodiscard]] bool luaRegisterBindings(LuaScriptHost &host, lua_State *state,
                                       std::string &error);
