...
bool Canvas2D::initialize(std::string &error) {
  ...
  program_ =
      resources_.createBuiltinProgram(BuiltinProgram::Textured2D, error);
  if (!program_) {
    shutdown();
    return false;
  }
...
bool Canvas2D::begin(const std::uint16_t viewId, const std::uint16_t width,
                     ...
                     const std::uint32_t clearRgba, std::string &error) {
  if (!whiteTexture_ || !sampler_ || !program_) {
  ...
  statistics_ = {};
  viewId_ = viewId;
  return commands_.configureView2D(
      View2DConfig{.id = viewId,
                   .width = width,
                   .height = height,
                   .clearRgba = clearRgba,
                   .frameBuffer = {}},
...
bool Canvas2D::solid(const Rect2D &destination, const std::uint32_t rgba,
  ...
  return add(whiteTexture_, destination, {}, rgba, blend, scissor);
}

bool Canvas2D::image(const TextureHandle texture,
                     const Rect2D &destination,
                     const TextureRegion2D &source, const std::uint32_t rgba,
                     const BlendMode blend, const ScissorRect scissor) {
  return add(texture, destination, source, rgba, blend, scissor);
}
...
bool Canvas2D::ninePatch(const TextureHandle texture,
                         const Rect2D &destination,
  ...
  const float horizontalScale =
      std::min(1.0F, destination.width /
                         std::max(border.left + border.right, 0.0001F));
  const float verticalScale =
      std::min(1.0F, destination.height /
                         std::max(border.top + border.bottom, 0.0001F));
  const float xs[4] = {
      destination.x,
  ...
  const float us[4] = {
      source.u0, border.center.u0, border.center.u1, source.u1,
  };
  const float vs[4] = {
      source.v0, border.center.v0,
      border.center.v1, source.v1,
  };
...
bool Canvas2D::circle(const float centerX, const float centerY,
  ...
  for (int index = 0; index < segments; ++index) {
    const float angle0 =
        2.0F * std::numbers::pi_v<float> * index / segments;
    const float angle1 =
        2.0F * std::numbers::pi_v<float> * (index + 1) / segments;
    if (!batch_.addTriangle(
            key,
            Triangle2D{
                .a = {.x = centerX,
                      .y = centerY,
...
bool Canvas2D::line(const float startX, const float startY, const float endX,
  ...
  return imageTransformed(
      whiteTexture_, (startX + endX) * 0.5F, (startY + endY) * 0.5F, length,
      width, 0.5F, 0.5F, std::atan2(deltaY, deltaX), {}, rgba, blend, scissor);
}
...
bool Canvas2D::circleOutline(const float centerX, const float centerY,
  ...
  for (int index = 0; index < segments; ++index) {
    ...
    if (!line(centerX + std::cos(angle0) * radius,
              ...
              centerY + std::sin(angle1) * radius, width, rgba, blend,
              scissor))
      return false;
  }
  return true;
...
bool Canvas2D::flush(std::string &error) {
  ...
  for (const QuadDrawRange &range : batch_.draws()) {
    ...
    success =
        commands_.submit(TransientDraw{
                             .viewId = viewId_,
...
bool Canvas2D::add(const TextureHandle texture, const Rect2D &destination,
                   const TextureRegion2D &source, const std::uint32_t rgba,
                   const BlendMode blend, const ScissorRect scissor) {
  if (!texture || !program_ || !positive(destination) ||
