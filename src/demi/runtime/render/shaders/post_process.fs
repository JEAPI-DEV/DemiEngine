...
void main() {
  ...
  glow += max(texture(texture0, fragTexCoord+vec2(texel.x, 0.0)).rgb-vec3(bloomThreshold), vec3(0.0));
  glow += max(texture(texture0, fragTexCoord-vec2(texel.x, 0.0)).rgb-vec3(bloomThreshold), vec3(0.0));
  glow += max(texture(texture0, fragTexCoord+vec2(0.0, texel.y)).rgb-vec3(bloomThreshold), vec3(0.0));
  glow += max(texture(texture0, fragTexCoord-vec2(0.0, texel.y)).rgb-vec3(bloomThreshold), vec3(0.0));
