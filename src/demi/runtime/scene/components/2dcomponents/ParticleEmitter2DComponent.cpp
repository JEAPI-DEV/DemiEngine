...
void ParticleEmitter2DComponent::parse(const nlohmann::json &json,
                                       Entity &entity) {
  ParticleEmitter2DComponent component;
  component.emissionShape =
      scene_loading::stringOr(json, "emission_shape", "point");
  if (auto value = scene_loading::vec2Field(json, "emission_size"))
  ...
  if (auto value = scene_loading::vec2Field(json, "velocity_min"))
    component.velocityMin = *value;
  if (auto value = scene_loading::vec2Field(json, "velocity_max"))
    component.velocityMax = *value;
  if (auto value = scene_loading::vec2Field(json, "gravity"))
