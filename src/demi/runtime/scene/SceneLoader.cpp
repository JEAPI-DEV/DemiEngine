...
[[nodiscard]] bool validateLoadPath(const std::filesystem::path& path, const char* label, std::string& error) {
...
[[nodiscard]] const SceneEntry* findSceneEntry(const ProjectData& project, const std::string& sceneId) {
...
std::optional<LoadedProject> loadProject(const std::filesystem::path& projectPath, std::string& error) {
  ...
  const std::optional<scene_loading::Json> projectJson = scene_loading::readJsonFile(projectPath, error);
  if (!projectJson.has_value()) {
    return std::nullopt;
  }

  std::optional<ProjectData> project = scene_loading::parseProjectData(projectPath, *projectJson, error);
...
std::optional<World> loadScene(const ProjectData& project, const std::string& sceneId, std::string& error) {
  ...
  const std::optional<scene_loading::Json> sceneJson = scene_loading::readJsonFile(scenePath, error);
  if (!sceneJson.has_value()) {
    return std::nullopt;
  }

  World world = scene_loading::parseSceneWorld(scenePath, *sceneJson);
  if (const std::optional<std::string> hud = scene_loading::stringField(*sceneJson, "hud")) {
