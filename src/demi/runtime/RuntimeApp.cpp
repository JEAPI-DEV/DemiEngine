...
constexpr std::array<KeyMapping, 64> KeyMap{{
  ...
  {KEY_SPACE, "space"}, {KEY_ENTER, "return"}, {KEY_ESCAPE, "escape"}, {KEY_TAB, "tab"},
  {KEY_BACKSPACE, "backspace"}, {KEY_UP, "up"}, {KEY_DOWN, "down"},
...
int runProject(const RuntimeOptions& options) {
  ...
  while (running && !WindowShouldClose()) {
    ...
    stepSimulation(loaded, luaHost, input, audioSystem, mediaSystem, networkSystem, dt, static_cast<float>(fixedStep), fixedAccumulator, running);
    ...
    if (use3D) {
      ...
      renderer3D.beginFrame(camera != nullptr ? *camera : fallbackCamera3D, activeCamera3DPosition(loaded.world), width, height);
      renderer3D.drawWorld(loaded.world);
      renderer3D.drawHud(loaded.world);
      renderer3D.endFrame();
    } else {
      const Camera2DComponent* camera = activeCamera(loaded.world);
      renderer2D.beginFrame(camera != nullptr ? *camera : fallbackCamera2D, activeCameraPosition(loaded.world), width, height);
