  ...
  if (auto children = node.find("children");
    ...
    for (Json &child : *children)
      if (Json *found = findNode(child, id))
        return found;
  if (auto elements = node.find("elements"); elements != node.end() && elements->is_array())
    for (Json &child : *elements)
      if (Json *found = findNode(child,id)) return found;
  return nullptr;
}
...
const Json *findNode(const Json &node, const std::string_view id) {
  ...
  if (auto elements = node.find("elements"); elements != node.end() && elements->is_array())
    for (const Json &child : *elements)
      if (const Json *found = findNode(child,id)) return found;
  return nullptr;
}

bool eraseNode(Json &node, const std::string_view id) {
  auto children = node.find("children");
  if (children == node.end()) children = node.find("elements");
...
bool EditorHudDocument::createNode(const std::string_view type,
  ...
  Json *root = replacement.contains("root") ? &replacement["root"] : &replacement;
  Json *parent = root == nullptr
                     ? nullptr
                     : (parentId.empty() || (!replacement.contains("root") && parentId=="ui_root"))
                         ? root : findNode(*root,parentId);
  if (parent == nullptr) {
  ...
  const auto key=parent==&replacement && parent->contains("elements")?"elements":"children";
  if (!parent->contains(key) || !(*parent)[key].is_array()) (*parent)[key]=Json::array();
...
bool EditorHudDocument::setNodeField(const std::string_view id,
  ...
  if (!replacement.contains("root") && id=="ui_root") {
    const auto key=replacement.contains("children")?"children":"elements";
    replacement["root"]={{"id","ui_root"},{"type","container"},
      {"anchor_min",{0,0}},{"anchor_max",{1,1}},
      {"children",replacement.value(key,Json::array())}};
  ...
  // Null erases the field so shorthand presets (dock/stack) can clear the
...
const Json *EditorHudDocument::authoredNode(const std::string_view id) const {
  ...
  if (root != document_.json().end()) return findNode(*root,id);
