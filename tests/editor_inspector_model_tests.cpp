...
int main() {
  ...
  const auto scale = std::ranges::find_if(common, [](const auto &field) {
    return field.field->name == "scale";
  });
  ...
  const auto field = std::ranges::find(descriptor->fields, "position",
  ...
  assert(schema["properties"]["position"]["x-demi-editor-label"] ==
         "Position");
  return 0;
}
